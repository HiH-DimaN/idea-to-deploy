...
Все одиннадцать хуков срабатывают сразу — перезапуск Claude Code не нужен. Два v1.5.1 enforcement-хука активны только внутри методологического репозитория (детект через `.claude-plugin/plugin.json`); на сторонних проектах это no-op. Два v1.17.0 safety-хука (`careful.sh`, `freeze.sh`) активируются по запросу пользователя. Pre-flight хук работает на любом проекте с распознанной директорией памяти; если памяти нет, он инжектит пустой блок контекста без предупреждения.
