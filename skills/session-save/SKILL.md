...
metadata:
  ...
  version: 1.0.0
  category: workflow
  tags: [session, memory, context, persistence, continuity]
---
...
Это не заменяет явный вызов в конце сессии, а дополняет его — чтобы при резком закрытии терялся минимум контекста.
